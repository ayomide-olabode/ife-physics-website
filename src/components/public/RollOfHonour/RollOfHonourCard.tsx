  ...
  return (
    <article className="border border-gray-200 bg-white overflow-hidden">
      ...
      <div className="space-y-2 p-4">
        <h3 className="text-base font-semibold text-brand-navy leading-snug">{entry.fullName}</h3>
        <p className="text-sm text-gray-600">{entry.registrationNumber}</p>
        <p className="text-sm text-gray-600">{entry.programme}</p>
        {entry.degree ? <p className="text-sm text-gray-600">{entry.degree}</p> : null}

        <div className="pt-1">
          <div className="mb-2 flex items-center justify-between text-sm">
            <span className="font-medium text-gray-600">CGPA</span>
            <span className="font-semibold text-brand-navy">{entry.cgpa.toFixed(2)} / 5.00</span>
          </div>
