...
import {
  ...
  archiveSpotlight,
  deleteSpotlight,
} from '@/server/actions/spotlight';
import { toastSuccess, toastError } from '@/lib/toast';

export function SpotlightStatusActions({ id, status }: { id: string; status: PublishStatus }) {
  ...
  const actions: {
  ...
  }[] = [
    {
      ...
      onClick: () =>
        setConfirmAction({
          title: 'Publish?',
          description: 'This will make it publicly visible on the spotlight sections.',
          confirmText: 'Publish',
          destructive: false,
          action: () => publishSpotlight(id),
        }),
    },
    {
      label: 'Unpublish',
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        ...
        if (confirmAction.title === 'Delete?') {
          router.push('/dashboard/communication/spotlight');
        } else {
          router.refresh();
        }
      } else {
