...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadDir = path.join(process.cwd(), 'public/uploads/legacy-gallery');
    ...
    const fileUrl = `/uploads/legacy-gallery/${filename}`;

    return NextResponse.json({ ok: true, url: fileUrl });
  } catch (err: unknown) {
    console.error('Legacy Gallery image upload error:', err);
